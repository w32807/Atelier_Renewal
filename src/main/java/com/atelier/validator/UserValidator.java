...
public class UserValidator implements Validator{
	...
	public void validate(Object target, Errors errors) {
		UserRequestDto userRequestDto = (UserRequestDto) ((UserVo) target).getUser();
		...
		try {
			...
			// ValidationUtils.rejectIfEmptyOrWhitespace는 target 기준이다.
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "shipping.saAddrHead", "confirm.shipping");
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "shipping.saAddrDetail", "confirm.shipping");
	...
	private Errors emailValidChk(UserRequestDto userRequestDto, Errors errors) throws NullPointerException{
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "user.cmEmail", "confirm.email");
		
		if(!ValidUtils.emailChk(userRequestDto.getCmEmail())) {
			errors.rejectValue("user.cmEmail","confirm.email");
			return errors;
		}
		
		if(service.emailDplChk(userRequestDto.getCmEmail()) > 0) {
			errors.rejectValue("user.cmEmail","duplicate.email");
			return errors;
		}
	...
	private Errors pwdValidChk(UserRequestDto userRequestDto, Errors errors) throws NullPointerException{
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "user.cmPwd", "confirm.pwd");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "user.conCmPwd", "confirm.pwd");
		
		if(!ValidUtils.pwdChk(userRequestDto.getCmPwd()) || !userRequestDto.getCmPwd().equals(userRequestDto.getConCmPwd())) {
			errors.rejectValue("user.cmPwd","confirm.pwd");
			return errors;
		}
	...
	private Errors nameValidChk(UserRequestDto userRequestDto, Errors errors) throws NullPointerException{
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "user.cmName", "confirm.name");
		
		if(!ValidUtils.nameChk(userRequestDto.getCmName())) {
			errors.reject("user.confirm.name");
			return errors;
		}
	...
	private Errors nickValidChk(UserRequestDto userRequestDto, Errors errors) throws NullPointerException{
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "user.cmNick", "confirm.nick");
	...
	private Errors phoneNumValidChk(UserRequestDto userRequestDto, Errors errors) throws NullPointerException{
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "user.cmPhone", "confirm.phoneNum");
